...
==== Local Overrides
In order to run the service, overriding the latest images with your locally built changes, you can make use of the `docker-compose*-local.yml` files. If you are testing updates to an individual handler, you can include the relevant `docker-compose*-local.yml` file in addition to the main `docker-compose.yml` file when running docker-compose:

[source, shell script]
----
docker-compose -f docker-compose-gene-list-local.yml -f docker-compose.yml
----

You can also test local changes in the importer service by including the `docker-compose-service-local.yml` file.